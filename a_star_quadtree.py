...
def a_star_quadtree(start_m, goal_m, qtm, movement='8n', occupancy_cost_factor=3):
    ...
    candidates.put((0, None, start, occupancy_cost_factor))   # store (distance, previous-tile, current-tile)
    while candidates:
        dis, prev_node, curr_node, weight = candidates.get()
        ...
        for til, deltacost in movements:
            ...
            if til not in path_record:
                deltacost=weight*dis
                candidates.put((dis + deltacost, curr_node, til, weight))
